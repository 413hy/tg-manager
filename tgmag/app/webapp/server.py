    ...
    return web.json_response(
        {
            ...
            "status": {
                "accounts": total or 0,
                "usable": usable or 0,
                "connected": len(client_pool.clients),
...
async def api_login_start(request: web.Request) -> web.Response:
    ...
    return web.json_response({"ok": True, "login_id": login_id, "message": "验证码已发送"})
    ...
    try:
        if pending.get("needs_password"):
            ...
            try:
                ...
                if not password:
                    return web.json_response({"ok": True, "needs_password": True, "message": "该账号需要 2FA 密码"})
                session_str, me = await account_ops.complete_password_login(pending["client"], password)
    except SessionPasswordNeededError:
        return web.json_response({"ok": True, "needs_password": True, "message": "该账号需要 2FA 密码"})
...
async def api_account_action(request: web.Request) -> web.Response:
    ...
    if action == "service_check":
        ...
        return web.json_response({"ok": True, "message": f"777000 检查完成，新增 {inserted} 条"})
        ...
        return web.json_response(
            {
                ...
                "message": (
                    ...
                    f"777000 新增 {service_inserted} 条"
                ),
...
async def api_account_twofa(request: web.Request) -> web.Response:
    ...
    if action == "confirm":
        code = str(data.get("code") or "").strip()
        pending = request.app["pending_twofa"].pop(str(account_id), None)
        try:
            await client(functions.account.ConfirmPasswordEmailRequest(code))
        except Exception as exc:
            if pending is not None:
                request.app["pending_twofa"][str(account_id)] = pending
            raise web.HTTPBadRequest(text=str(exc)) from exc
        async with sessionmaker() as session:
            if pending:
    ...
    try:
        ...
        request.app["pending_twofa"][str(account_id)] = {
...
async def api_account_login_email(request: web.Request) -> web.Response:
    ...
    if action == "send":
        ...
        try:
            ...
            raise web.HTTPBadRequest(text=str(exc)) from exc
        request.app["pending_login_email"][str(account_id)] = email
        return web.json_response({"ok": True, "needs_code": True, **sent, "message": "登录邮箱验证码已发送"})
    if action == "confirm":
        code = str(data.get("code") or "").strip()
        email = request.app["pending_login_email"].pop(str(account_id), None)
        try:
...
async def api_targets_update(request: web.Request) -> web.Response:
    ...
    async with sessionmaker() as session:
        ...
        if action == "add":
            ...
            if not target_ref:
                raise web.HTTPBadRequest(text="target_ref required")
            session.add(
                AllowedTarget(
...
async def api_rate_update(request: web.Request) -> web.Response:
    ...
    async with sessionmaker() as session:
        ...
        if row is None:
            ...
            session.add(row)
        row.max_actions = int(data["max_actions"])
        row.per_seconds = int(data["per_seconds"])
        row.jitter_min = int(data.get("jitter_min") or 0)
        row.jitter_max = int(data.get("jitter_max") or 0)
